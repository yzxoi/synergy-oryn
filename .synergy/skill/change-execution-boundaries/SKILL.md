...
Apply revocable workflow ownership to knowledge mutations as well as execution and publication. Historical source bindings establish provenance, not current authority. Test pause, takeover, cancellation, replaced Attempts and ownership epochs against both creating and withdrawing durable knowledge.

For Case-wide time limits, derive deadlines from durable Case identity rather than individual Attempts, and combine admission checks with lifecycle-owned cancellation of already-running work. Keep notification network calls out of the enforcement loop. Test an expired active callback and an actual background process, fresh startup, unchanged QA admission, duplicate notification prevention and a concurrently completed Attempt. Distinguish wall-clock expiry and cleanup latency from model-token accounting and OS CPU quotas.

For writable check outputs, materialize only the fixed Git inputs into an isolated experiment and permit writes solely to Host-selected output directories with no tracked-source overlap or symlink ancestors. Test a real build and subsequent execution, ignored-file exclusion, source/metadata denial, repository profile restrictions, independent experiment outputs and cleanup. Expose allowed profiles to the engineering model; scripted fixture knowledge is not product discoverability. Do not label requested overlays as applied when no patch was materialized.

For pre-provisioned dependencies, pin a versioned manifest digest and source dependency inputs, verify each blob before use, and copy into each experiment instead of mounting a shared writable install. Test actual package resolution on the deployment platform, workspace link rebinding, chained link escapes, lock/patch invalidation and dependency write denial. Keep snapshot directories out of model-facing profile discovery and distinguish byte integrity from installation provenance.

For OS resource limits, verify applied kernel controls before untrusted execution and test exhaustion plus detached-child cleanup. Start trusted quota/bootstrap runners from an installation-owned directory and home so candidate preload/configuration cannot execute before sandbox entry. Keep supervisor/bus environment out of the sandbox and distinguish per-command, aggregate-service and model-worker accounting. A lifetime bound independent of the Host limits crash-orphan duration; it does not replace recovery auditing.

For Oryn trusted-local changes, run the Oryn sandbox, shell, config and success-pipeline suites. Verify actual outside-worktree writes and network access in explicitly trusted mode, unchanged default containment, Host-only selection, accurate unsandboxed wrapper metadata and receipt mode provenance. Preserve timeout/cancellation and reject unavailable declared capabilities instead of silently stripping them.
