...
For Oryn pipeline changes, distinguish PR merge-base scope from the live integration target. Exercise target-only advancement, stale contributor heads, durable comment replacement, author re-review versus writer-only repair, and actual grouped notification delivery. Step budgets need real model/tool/process lifecycle evidence, cumulative resume accounting and idle-time exclusion; Case creation timestamps are not execution evidence. Use `test/oryn/step-budget.test.ts`, `integration.test.ts`, `candidate.test.ts`, `github-review.test.ts` and `operator-notifications.test.ts` with the existing pipeline suites.

Recovery acceptance must drive the real Session loop after a terminal rollout and assert a structured result, not manually materialize its Inbox item. Validate fixed-object cache hits without credentials and incremental fetch in a private Git configuration; isolation must not force repeated whole-history downloads.
