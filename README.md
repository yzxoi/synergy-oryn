...
GitHub intake preserves existing Issue identities, reviews contributor PRs on their current commits, and accepts permission-checked `@oryn review`, `@oryn fix` and `@oryn stop` commands. Independent discoveries enter private reproduction before public Issue creation. Target-specific dependencies, native containment and live App credentials require deployment acceptance; deterministic tests do not establish target-VPS readiness.
