...
Implementation status: integration is in progress against the [implementation proposal](docs/decisions/proposed/architecture/2026-09-07-synergy-oryn.md), with [research background](docs/research/2026-09-07-maintenance-automation-proposal.md) and a [developer handoff](docs/research/2026-09-07-synergy-oryn-handoff.md). Local tests cover configured Feishu ingress and explicit delivery, model-tool schemas, role-scoped reads, Boss worker handoff and result replay. Automatic Case startup, complete crash recovery, trusted candidate execution, independent delivery gates and GitHub lifecycle integration still require work; the current implementation is not deployment-ready. Live Feishu canary, GitHub App publishing and VPS verification remain separate environment acceptance tasks.
