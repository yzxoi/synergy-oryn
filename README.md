...
Implementation status: integration is in progress against the [implementation proposal](docs/decisions/proposed/architecture/2026-09-07-synergy-oryn.md), with [research background](docs/research/2026-09-07-maintenance-automation-proposal.md) and a [developer handoff](docs/research/2026-09-07-synergy-oryn-handoff.md). Local tests cover configured Feishu ingress, per-root follow-up Case intake, source-authorized QA reads and explicit delivery, model-tool schemas, deterministic chat/embedding model-driven QA intake, Boss reproduction with report-driven human handoff, and complete scripted edit/Host-commit/independent-verify/review/PR-ready and review-driven repair branches with captured GitHub and Feishu transports, persisted handoff reasons and recoverable subscriber notifications, role-scoped reads, reserved Boss worker creation with index/worktree/Inbox recovery, bounded consumed-turn continuation and result replay, fresh Attempts and engineering tasks after explicit ownership resume, separate-process concurrent investigation recovery including lost Issue creation responses, PR success and repair recovery after lost Draft/refresh/ready responses with a further completed-state restart, recoverable Feishu Case startup in an explicitly configured repository, structured review replay with per-domain delivery checks, assigned candidate Git verification at acceptance, stage admission and delivery, assignment-owned run references and independent verification reports, separate version-pinned worker checkouts with source-change invalidation, opt-in GitHub type/progress labels with preserved human priorities and recoverable label receipts, Host-rendered issue/PR descriptions with actual Git diff scope diagrams and accepted execution/review evidence, Case-bound GitHub Draft-to-ready transitions and ambiguous-response reconciliation with mocked network transport, bounded check execution with central profile quotas, read-only source sandboxes and disposable check homes, and assignment-bound worker Bash with private Git metadata, restricted networking, Session-scoped process control, tracked background cleanup and Case pause/takeover/cancel with queued-task suspension. Complete worker crash recovery, trusted candidate execution, independent delivery gates and GitHub lifecycle integration still require work; the current implementation is not deployment-ready. Live Feishu canary, GitHub App publishing and VPS verification remain separate environment acceptance tasks.
