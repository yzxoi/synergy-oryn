...
Oryn is dormant until explicitly enabled. In Settings → Oryn, select a configured GitHub repository, its trusted server checkout and target branch, and the Feishu chat or topic for human intervention. Enable existing-issue backfill and independent PR review as needed; automatic coding is an explicit opt-in. Advanced execution profiles and budgets live in the installation’s `120-runtime.jsonc` domain. Installations can explicitly select [trusted local execution](docs/operations/oryn-deployment.md#trusted-local-execution) for unrestricted filesystem and network execution under the runtime OS user; sandbox remains the default. Project configuration cannot enable Oryn or change its business policy; the built-in Oryn agent identities cannot be replaced by config, plugins or external adapters. While disabled, all Synergy channel, Boss, Feishu, and GitHub behavior is unchanged.
...
GitHub intake preserves existing Issue identities, reviews contributor PRs on their current commits, and updates one durable review comment, waits quietly for author fixes, and accepts permission-checked `@oryn review`, `@oryn fix` and `@oryn stop` commands. Execution uses per-step budgets and fixed-commit workspaces with optional frozen-lock dependency installation. Optional emoji labels track both Oryn-created work and existing or future contributor Issues/PRs, preserving unrelated labels and human priorities. Independent discoveries enter private reproduction before public Issue creation. Target-specific dependencies, native containment and live App credentials require deployment acceptance; deterministic tests do not establish target-VPS readiness.
