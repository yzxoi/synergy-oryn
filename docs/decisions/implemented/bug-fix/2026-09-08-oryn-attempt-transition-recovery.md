...
The store writes a version 2 transition intent under the Case, keyed by the previous Attempt identity for rework or the ownership epoch for a human resume, before changing either Attempt. It reserves the replacement identity and complete initial record, exact transition inputs, transition purpose, expected control state, ownership epoch, expected Case revision and resulting repair counters. Under the existing Case lock, replay validates the same input and epoch, reuses the reserved replacement, supersedes the previous Attempt once and advances the Case pointer and revision once. A completed replay repairs the Case index without incrementing counters or discarding later replacement evidence.

Runtime startup reconciles these intents before Session recovery can wake model work. Completed historical transitions whose replacements are no longer active are skipped. Incomplete transitions whose Case revision changed are paused for inspection rather than overwriting intervening decisions. Inactive ownership resumes only when a persisted human-resume intent matches its control state, revision and epoch. Subsequent human takeover or cancellation prevents that intent from activating work.

The intent is local recovery metadata in the existing Oryn store, not another execution queue. Attempt v1 and Case v2 formats remain unchanged. The central `20260908-oryn-attempt-transition-purpose` migration upgrades version 1 intents to rework intents with an active control precondition, preserving their reserved identities and effects. No intent is fabricated for historical transitions lacking one.
