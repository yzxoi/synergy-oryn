...
Pause preserves queued task Inbox entries; resume asks the existing SessionDrive to arbitrate them. Consumed interrupted turns use [bounded task recovery](2026-09-08-oryn-interrupted-task-recovery.md). Takeover/cancel invalidate worker epochs; explicit resume allocates a fresh Attempt and task through [ownership recovery](2026-09-08-oryn-ownership-resume.md). QA is not cancelled merely because one associated Case stops.
