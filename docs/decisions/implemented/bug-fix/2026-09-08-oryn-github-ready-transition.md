...
The Host permits PR updates only to Case-bound PR numbers and persists the target before dispatch. Publication receipts pin the Attempt, repository, branch/base and optional-check setting; the owning migration is registered centrally and preserves old receipts without guessing missing targets. Legacy ready receipts cannot automatically finalize a candidate. Configuration or Attempt changes prevent recovery from granting a new ready result, including when two Attempts use the same SHA. Readiness reconciliation requires matching non-draft remote facts, current local evidence and any currently configured delivery check. It does not replay a remote mutation. Acknowledgement recovery restores the active candidate's ready disposition only for the receipt's Case revision and epoch. Ready notifications use a Host-generated publication conclusion identity; repeated model requests reuse the same notice, while a reauthorized conclusion after a pause has a new identity. Old candidates and inactive Cases cannot acquire new delivery effects. Poll recovery also repairs acknowledged actions interrupted before their local effects finished.

The ready completion path queues authorized reporter notices and immediately drains the outbox. A QA ready reply requires the acknowledged conclusion and reuses its Host text and identity; the model cannot manufacture a readiness claim. Before dispatch, the Host observes the exact open non-draft PR, App ownership, branch/base, head, independent CI and configured delivery check, then reevaluates the current candidate and review evidence. Unavailable or mismatching remote proof leaves the definitely unsent intent pending. A Case/Attempt-locked local claim follows transport readiness and remote verification, checking the conclusion, configuration digest and source authorization again. No Case lock spans network delivery: a takeover before the claim prevents sending, while a send already claimed has an in-flight outcome to reconcile. This local claim cannot prevent a later GitHub push or revoke a network request already dispatched.

Action schema version 4 persists a deterministic notification key before publication. The centrally registered ready-notification migration preserves legacy targets without inventing new keys. For an unchanged conclusion the earliest matching acknowledged receipt controls notification identity, preserving delivered and ambiguous legacy notices. A later Case version can notify after its predecessor was suppressed. These are result-delivery guarantees, not proof of live Feishu delivery or model reasoning quality.
