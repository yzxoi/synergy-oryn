...
The same success fixture also runs a review-driven repair round. The first candidate preserves values but shares the caller array; the independent candidate check fails and review records a stable open finding. Engineering publishes that review, rotates the Attempt and dispatches fresh workers. The second coder and reviewer retrieve the persisted prior review through the model tools. The coder reports the addressed finding, and the reviewer resolves it against fresh execution receipts. The simulated GitHub transport requires the updated candidate to descend from the published head and refreshes the existing PR. Assertions preserve the superseded Attempt, failed receipt and open review, check the persisted candidate's addressed findings, and permit only one final ready notification for the corrected SHA. The result tool forwards addressed findings to the owning service; submission remains distinct from the reviewer's evidence-backed resolution.
