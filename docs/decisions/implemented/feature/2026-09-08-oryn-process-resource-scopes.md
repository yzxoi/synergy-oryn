...
Provisioning stops the test account's existing manager before changing delegation, then reloads configuration and starts the manager. This avoids applying new delegation to an active user tree. Ubuntu 22.04 CI reports `219/CGROUP` when restarting its existing manager after a live configuration reload; the precise kernel failure remains unconfirmed. The upstream [delegated-service restart discussion](https://github.com/systemd/systemd/issues/24064) identifies retained subgroup state as a related failure mode, not proof of this runner's cause. Native execution tests remain the acceptance gate for the provisioning sequence.
