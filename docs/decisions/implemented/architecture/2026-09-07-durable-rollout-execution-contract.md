...
Transport cancellation first releases the pending upstream read, then awaits commitment of the bytes already received before closing the body artifact. Attempt finalization awaits both request and response bodies and shares one completion promise across concurrent callers. The recorder continues to reject writes after closure and genuine persistence failures. A deterministic blocked-read test covers this ordering; the [cancellation postmortem](../../../postmortem/0009-rollout-cancellation-closed-artifact.md) records the escaped race.
