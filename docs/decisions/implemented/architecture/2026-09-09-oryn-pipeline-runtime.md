...
PR input records distinguish head, target base and merge base. Host fetch reuses and pins fixed objects already present in the trusted cache, borrows that object store for incremental negotiation in a private credential-bearing repository, and changed-file classification and review use merge-base-to-head scope. Reviewer dispatch validates the assigned head and merge base rather than the moving target tip. Target-only advancement does not invalidate an otherwise unchanged review. Baseline and candidate checks each materialize the requested commit into an independent experiment; caller HEAD and uncommitted worker edits do not select its source. An explicit bounded test-only patch can be applied to both versions, and the receipt includes the resulting tree digest.
...
The Session loop materializes a queued new task before reading execution configuration from a sealed previous rollout. GitHub review sessions remain ineligible until the Host has assigned the current fingerprint and versions; admission is persisted before wakeup. Operator resume triggers GitHub recovery before ordinary Session recovery. Terminal rollouts without accepted reports receive a fresh task root in the same Session/worktree; interrupted rollouts retain normal steering. Recovery remains bounded. Oryn avoids generic file snapshots and optional activity-summary inference because fixed Git versions and durable workflow records supply its evidence. Equal environment failures in a repository can share one operator notification; source-specific clarification remains separate. Nonactive open work continues to receive accurate label projections.
