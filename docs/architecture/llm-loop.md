...
Host-registered Agent turn admission classifies workloads from the canonical rollout owner. Enabled Oryn engineering and worker Session bindings use the background class, including derived calls attributed to those Sessions; QA and unrelated owners retain ordinary admission. The existing pool limits background occupancy to `max(1, agentWorkers - 1)`, scans past blocked background entries for the oldest eligible turn and holds occupancy until worker release. Background submissions leave one aggregate admission position and up to one maximum-size request of queued bytes available to foreground work; the byte reserve is capped at half the configured waiting-byte budget. All submissions remain subject to the aggregate count and byte bounds. This is shared foreground capacity, not a dedicated QA process or a provider rate-limit guarantee. A one-worker pool cannot reserve independent capacity. Resize does not preempt owned turns, so reduced ceilings take effect as those turns release. Workload metadata stays in the Control Plane and is not part of the model or worker request.
