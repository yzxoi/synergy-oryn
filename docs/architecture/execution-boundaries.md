...
First-party Host admission providers can select an executor and bounded resource quotas before dispatch. ToolScheduler acquires these quotas atomically with global and executor capacity, and keeps them until registered physical execution settles. Quotas and callbacks remain in the Control Plane; resolver capability authorization still applies. See [Oryn check admission](../decisions/implemented/architecture/2026-09-08-oryn-check-resource-admission.md) for the check-specific integration and accounting limits.

Oryn can additionally place checks and worker Bash in installation-selected Linux resource scopes. The trusted scope runner validates applied kernel controls before the sandbox starts; physical cleanup stops the scope, including detached descendants. This does not change permission decisions or scheduler ownership. See [process resource scopes](../decisions/implemented/feature/2026-09-08-oryn-process-resource-scopes.md).
...
Learning proposal and withdrawal also require active Case ownership and budget. The current engineering root or a worker assigned to the current epoch and Attempt may mutate lessons; a persistent source binding alone is insufficient. Mutations validate under the Case lock, and withdrawal shares the promotion effect lock. See [learning ownership](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-ownership.md).

Promotion separately requires the active Attempt and its acknowledged delivery action, current installation policy and remote confirmation. Library preparation and remote checks run outside the Case lock; the Host revalidates the delivery snapshot under that lock before synchronous insertion and durable acknowledgment. See [learning delivery checks](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-current-delivery.md).

The Host pins each new proposal to repository, Attempt, ownership epoch, acceptance criteria, source commits and the digest of accepted evidence; promotion rechecks that source before preparation and commit. The immutable memory payload labels the lesson as a model proposal. See [learning provenance](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-provenance.md).
...
Host-owned check runners can pass a complete permission profile to `SandboxBackend.prepareWrapper`. Linux preserves it for the verified helper; macOS compiles explicit roots without adding the interactive user/toolchain defaults when `includePlatformDefaults` is false. Legacy wrappers, no-sandbox execution and Windows reject this explicit-profile path. Oryn checks use it for read-only source and disposable HOME/temp access with restricted networking. Explicit macOS profiles allow ancestor directory metadata needed for path traversal, without granting ancestor directory listings or file contents. See [Oryn check containment](../decisions/implemented/bug-fix/2026-09-08-oryn-check-containment.md).

Oryn check execution materializes the fixed commit into a private disposable Git checkout. Installation profiles select relative writable output directories, which must not overlap tracked/materialized source, symlink ancestors or protected metadata. Source and repository objects remain read-only; the plan shares only its own outputs, and physical completion precedes disposal. The runner does not import untracked dependencies, candidate Git configuration or credentials. Repository profile restrictions apply before scheduler admission, and engineering can inspect the allowed profiles through Case reads. Source-overlay plans fail because no overlay is applied. See [disposable build experiments](../decisions/implemented/feature/2026-09-08-oryn-disposable-build-experiments.md) for verification and remaining provisioning limits.

Oryn worker Bash receives a Host execution policy from the central ToolResolver after capability authorization. The policy requires the built-in local executor, an active assignment with matching epoch/Attempt/agent/workspace, and a supported explicit sandbox. Its prepare callback revalidates ownership before spawning. Reproduction and coding worktrees are writable before candidate freeze; frozen source remains read-only even under `full_access`. Commands use a private HOME and Git metadata view with read-only access to the repository object store; host Git configuration, credential variables and language startup variables are absent. Tracked background execution retains this policy until process-group termination and asynchronous scratch cleanup settle. See [Oryn worker shell containment](../decisions/implemented/bug-fix/2026-09-08-oryn-worker-shell-containment.md) for scope and remaining execution limitations.

Oryn worker process control is scoped to the immutable Session ownership recorded by LocalBashBackend in ProcessRegistry and retained in finished entries. The central resolver selects a Host process-access policy; local list/log/poll/input/kill/remove/clear cannot target another Session or an unowned process, and remote/custom process executors are rejected. Input requires the current writable assignment; own-process inspection and cleanup remain available after freeze or human handoff. The registry retains executor completion promises so kill/remove and stale-PID inspection cannot report completion before asynchronous cleanup. Ordinary interactive process listing keeps its existing scope. See [worker process ownership](../decisions/implemented/bug-fix/2026-09-08-oryn-worker-process-ownership.md).

Oryn credential-bearing Git publication runs in a Host-owned temporary bare repository, borrowing only the frozen candidate object store. It uses a Host-installed Git executable, explicit GitHub HTTPS URL, repository-scoped credential response, disabled redirects and an allowlisted environment. Candidate hooks, remotes, Git configuration and inherited credentials cannot configure this operation. Every command has a bounded output/time budget and its owned POSIX process group is terminated on cancellation and cleanup. Non-fast-forward rejection preserves the remote branch; other push transport failures remain ambiguous until the action ledger reconciles remote facts. See [isolated Git publication](../decisions/implemented/bug-fix/2026-09-08-oryn-isolated-git-publication.md).

Oryn installations may explicitly select [trusted local execution](../decisions/implemented/feature/2026-09-09-oryn-trusted-local-execution.md). Its Host shell policy authorizes an unwrapped command without reporting it as sandboxed. New engineering roots use full_access; assignment and publication checks remain separate. This is an explicit operator policy, never a fallback for sandbox startup failure.
