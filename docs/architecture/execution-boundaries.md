...
Learning proposal and withdrawal also require active Case ownership and budget. The current engineering root or a worker assigned to the current epoch and Attempt may mutate lessons; a persistent source binding alone is insufficient. Mutations validate under the Case lock, and withdrawal shares the promotion effect lock. See [learning ownership](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-ownership.md). Promotion separately requires the active Attempt and its acknowledged delivery action, current installation policy and remote confirmation. Library preparation and remote checks run outside the Case lock; the Host revalidates the delivery snapshot under that lock before synchronous insertion and durable acknowledgment. See [learning delivery checks](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-current-delivery.md). The Host pins each new proposal to repository, Attempt, ownership epoch, acceptance criteria, source commits and the digest of accepted evidence; promotion rechecks that source before preparation and commit. The immutable memory payload labels the lesson as a model proposal. See [learning provenance](../decisions/implemented/bug-fix/2026-09-08-oryn-learning-provenance.md).
