...
Oryn check commands use the shared asynchronous physical runner with a combined 64 KiB output limit per command. Truncated output produces inconclusive evidence and stops the remaining sequence; cancellation and timeout also stop further commands. This physical-runner integration does not enable an Oryn OS sandbox or replace its admission queue. See [check process lifecycle](../decisions/implemented/bug-fix/2026-09-08-owned-check-process-lifecycle.md).
