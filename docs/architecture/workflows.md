...
Host integrations can reserve a worker Session ID before calling `BossService.spawn`. Reserved creation repairs Session indexes, verifies the original parent/role/agent/instructions and reuses its registered worktree with matching base inputs. A Host request can require the previously bound Session to exist, preventing reconstruction with empty history. Boss assignment replay wakes a runnable persisted task without duplicating its delivery. The reserved identity option is not exposed to model tools or HTTP clients.
...
A task-report provider returns undefined for unowned tasks; its boolean result overrides ordinary report detection for owned tasks. Oryn requires structured results and quiesces invalidated assignments. Formal reviews use stable request identities, domain-specific findings and accepted-report delivery recovery; the result tool can read them and the check tool can read execution receipts. See [structured review delivery](../decisions/implemented/architecture/2026-09-08-oryn-structured-review-delivery.md).

Oryn Feishu Case submission starts a dedicated engineering Boss root in the configured repository Scope and delivers its initial task through Inbox. Runtime startup resumes incomplete creation using reserved identities and wakes existing runnable work. Boss roots still do not poll or continue unconditionally. Oryn can resume consumed interrupted engineering/worker turns through bounded Host-authored Inbox steering, preserving the task root and worktree; see [interrupted task recovery](../decisions/implemented/bug-fix/2026-09-08-oryn-interrupted-task-recovery.md). See [engineering startup](../decisions/implemented/architecture/2026-09-08-oryn-engineering-startup.md) for configuration validation and recovery.
...
Every dispatched Oryn worker uses a separate Boss worktree: repro/code start from baseline, verify/review from the frozen candidate. Check execution derives the worker directory from its Assignment and records initial Git state, rejecting mismatched/dirty lanes and invalidating source changes during execution. See [versioned worker workspaces](../decisions/implemented/architecture/2026-09-08-oryn-versioned-worker-workspaces.md) for recovery limits and the remaining executor containment work.

Oryn check commands use the shared asynchronous physical runner with a combined 64 KiB output limit per command. Truncated output produces inconclusive evidence and stops the remaining sequence; cancellation and timeout also stop further commands. This physical-runner integration does not enable an Oryn OS sandbox. See [check process lifecycle](../decisions/implemented/bug-fix/2026-09-08-owned-check-process-lifecycle.md).

Oryn check runs enter ToolScheduler with installation-owned heavy/profile quotas and require a matching live execution context before running. Read/propose actions use control-plane scheduling. Check cancellation and physical cleanup retain their resource counts until settlement; coder Bash and model-capacity accounting remain separate work. See [check resource admission](../decisions/implemented/architecture/2026-09-08-oryn-check-resource-admission.md).

Oryn publication replay and GitHub polling reconcile orphaned `prepared`/`in_flight` receipts under a per-Case publication lock, discover unknown artifact numbers from bounded App-authored repository listings and repair acknowledged Case links without repeating the external write. See [publication crash recovery](../decisions/implemented/bug-fix/2026-09-08-oryn-publication-crash-recovery.md) for uncertainty, concurrency and experiment limits. The [PR process-recovery experiments](../decisions/implemented/testing/2026-09-08-oryn-pr-process-recovery.md) exercise lost Draft, refresh and ready responses through real runtime death, independent checks/review and a second restart after completed delivery.

Oryn Attempt rotation reserves its replacement and counter effects in a durable transition intent before modifying Attempts. Replay preserves the replacement identity and repairs a published Case's index; startup reconciles unfinished transitions before Session wakeup and pauses revision conflicts. See [Attempt transition recovery](../decisions/implemented/bug-fix/2026-09-08-oryn-attempt-transition-recovery.md).

Explicit Oryn resume after takeover, cancellation or human handoff creates a fresh Attempt and Host-authored task in the existing engineering Session. Central admission waits for that epoch's task in Inbox or canonical history; old workers remain invalid and obsolete pending engineering input is removed before first delivery. Ordinary pause/resume preserves the current Attempt. See [ownership resume](../decisions/implemented/bug-fix/2026-09-08-oryn-ownership-resume.md).
