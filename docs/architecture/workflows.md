...
Oryn publication replay and GitHub polling reconcile orphaned `prepared`/`in_flight` receipts under a per-Case publication lock, discover unknown artifact numbers from bounded App-authored repository listings and repair acknowledged Case links without repeating the external write. See [publication crash recovery](../decisions/implemented/bug-fix/2026-09-08-oryn-publication-crash-recovery.md) for uncertainty, concurrency and experiment limits.
