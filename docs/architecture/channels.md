...
Installation-enabled label projection covers both App-created artifacts and contributor threads admitted under the current repository/account binding. The Host rechecks Case control and source fingerprint before each write; contributor PR labels additionally require the current head and target repository/base. Emoji display names map to stable persisted label IDs. Draft or review-disabled PRs can be retained for labels without starting engineering. See [tracked labels](../decisions/implemented/feature/2026-09-09-oryn-tracked-emoji-labels.md).
