...
Oryn action receipts use schema version 3; `20260908-oryn-label-target` adds pinned label targets after the version 2 ready-target migration. Ready actions pin their Attempt, repository, branch/base and delivery-check setting before dispatch. The `20260908-oryn-ready-target` central migration preserves legacy receipts without inferring missing targets; those records cannot automatically finalize readiness. See the [GitHub ready decision](../decisions/implemented/bug-fix/2026-09-08-oryn-github-ready-transition.md).
