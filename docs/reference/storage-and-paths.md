...
Oryn stores version 1 Attempt transition intents under `oryn/cases/<case>/attempt_transitions/<previous-attempt>`. Each pins the replacement identity and initial state, exact transition inputs, ownership epoch, expected Case revision and resulting counters before rotation writes begin. Keep this record family with Case and Attempt backups. Startup reconciles it before waking Sessions; it is recovery metadata in the existing store, not an execution queue. See [Attempt transition recovery](../decisions/implemented/bug-fix/2026-09-08-oryn-attempt-transition-recovery.md).
