...
GitHub needs only outgoing HTTPS. Incremental changes continue while the open backlog is scanned. An exact standalone `@oryn review` (also `re-review` or `re-run`) is accepted from a repository writer or the PR author. `@oryn fix` (also `autofix`) and `@oryn stop` require repository write, maintain or admin permission. Quoted commands and bot comments are not authority. Explicitly authorized repair can append commits to an existing same-repository branch after verifying its expected head and ancestry; inaccessible fork branches use a credited continuation PR. Original commits remain intact. Humans merge every PR.

A review freezes head, target base and merge base separately; changed files come from merge base to head. Advancing only the target does not rerun the review. A general reviewer selects specialist work from concrete risk. One durable App comment is updated with the reviewed head, findings and bounded prior review history. Actionable findings move work to `waiting_author`, freeing active capacity. It includes each domain's recommendation, evidence assessment, questions and limitations. A review does not write a successful delivery check. Missing reproduction or environment evidence transfers work to a human. GitHub-origin handoffs and gated ready results use the selected Feishu destination; ordinary progress remains silent.

Discoveries keep their source Case and version. Independent or blocking bugs create a private reproduction Case before any public Issue. Security findings stay private, environment gaps require environment repair, and current-change defects stay in the existing review. Exact local observations deduplicate; `maxDiscoveryDepth` and `maxDescendants` default to 2 and 8. Descendants retain bounded lineage and use their own execution steps; a root requiring human attention blocks descendants. Semantic deduplication across unrelated wording and aggregate token accounting still require operator judgment and monitoring.

Back up the full runtime state, including `oryn/github`, `oryn/github_cursors`, `oryn/github_owned` and `oryn/discoveries`. A lost Review response leaves an ambiguous receipt; polling searches the App's exact comment revision marker before settling it and never automatically resends an uncertain publication. Retain old receipts when updating PRs. Poll snapshots retain the latest 100 combined comments/reviews; use GitHub for complete discussion history.
...
## Dependencies from the Assigned Commit

For trusted local execution, set the check profile to `dependencies: "install"`. Each experiment installs dependencies from its own fixed commit using the committed lockfile and frozen-lock package-manager command. Only the download cache is shared, not `node_modules`; baseline and candidate may therefore use different lockfiles. Bun, pnpm, npm and Yarn are detected, and the selected command must be allowlisted. `installTimeoutSeconds` defaults to 1800. Missing or ambiguous locks and installation failures are environment gaps requiring operator action. The optional sealed mode below remains useful for offline or contained checks.

A verifier can supply a bounded test-only unified patch when proposing `oryn_check`. The same patch can be tested against baseline and candidate; the receipt pins both the commit and resulting tree. Edits in an Agent worktree are never implicitly copied into an experiment.

Before delivery the Host fetches the target and checks actual mergeability. A cleanly compatible target does not trigger a rebase. For conflicts, start a bounded rework round, ask the code worker to invoke `oryn_result` with `kind: "integration", action: "start"` before editing, resolve semantics, then commit through the Host. Fresh independent verification and review are required for the new candidate. Merge remains a human action.

## Sealed Dependency Inputs

Prepare dependencies on the same Linux architecture and exact Bun version as the runtime, in a separate reviewed clean checkout. For offline or contained checks, select `dependencies: "snapshot"`; the runtime materializes the selected sealed snapshot without installing packages. For a repository whose dependencies work without lifecycle scripts, use `bun install --frozen-lockfile --ignore-scripts` in that checkout, then confirm the intended checks have the required dependencies. This operator step may require outbound access; do not run it with GitHub App keys, Feishu credentials or the production runtime home in its environment. Native dependencies requiring install scripts need a separately reviewed preparation step; a snapshot does not prove which commands created its bytes.
...
- Set `oryn.limits.heavyConcurrency` for check processes (runtime default 2) and `maxStepMinutes` for each logical execution step (default 360). Total Session-worker counts, `lightConcurrency` and model-token limits remain unenforced; the corresponding schema descriptions are not proof of enforced limits. Set `execution.agentWorkers` to at least 2 for foreground capacity reservation; 4 is a starting point to measure on the target VPS.
...
- Step time accumulates only during model/tool execution, including background processes and dependency installation. Overlapping activity in one step counts once. Queue, child-wait, human-wait and downtime do not count. Repeated calls and process restarts retain recorded totals; ownership resume carries the prior budget identity. Independent review domains have independent budgets. The runtime checkpoints active steps every 15 seconds, persists final totals, and hands off exhausted work through the existing cancellation/process cleanup path. An abrupt crash can lose at most the uncheckpointed interval; this is not CPU-time or token accounting.
...
Upgrade migrates `maxCaseMinutes` to `maxStepMinutes: 360`, preserving an explicitly configured step limit. Case age is not imported as execution time. Existing human handoffs remain stopped until an operator resumes them; exhausted steps need a larger limit before resuming. The settings panel exposes both step minutes and the maximum active Cases. Verify budgets, cancellation, real background cleanup and resume with `bun test test/oryn/step-budget.test.ts test/oryn/worker-start.test.ts test/oryn/shell.test.ts` from `packages/synergy`.
...
Run `bun run test test/oryn/worker-start.test.ts` from `packages/synergy` for reserved-worker and consumed-turn recovery. The suite includes repeated interrupted recovery, explicit pause/resume, engineering report consumption, a scripted model completing through the actual result tool, and a held engineering lease during Host handoff. Fixtures preserve the worker and worktree; interrupted rollouts continue their root, while terminal rollouts missing a report receive a fresh root; they simulate interrupted durable state within an isolated test runtime rather than killing and restarting the complete server process.
